...
namespace fds_planner {
  ...
  bool FDSPlanner::makePlan(const geometry_msgs::PoseStamped& start, 
    ...
    while (iter < max_iterations) {
      ...
      double best_cost = 256.0; 
      double min_angle_diff = 1e9;
      geometry_msgs::PoseStamped best_next_pose = current_pose;
      bool found_valid_step = false;

      // 3. FDS 核心：在扇区内进行方向采样搜索
      for (int i = 0; i <= num_samples; ++i) {
        // 强制一个采样点直接指向目标以避免 Z 字形走位
        double sample_angle = (i == num_samples) ? angle_to_goal : (angle_to_goal + (i - num_samples / 2.0) * (search_range / num_samples));
        ...
        if (costmap_->worldToMap(next_x, next_y, mx, my)) {
          ...
          double angle_diff = fabs(sample_angle - angle_to_goal);
          while (angle_diff > M_PI) angle_diff -= 2.0 * M_PI;
          angle_diff = fabs(angle_diff);

          // 核心优化：
          // 1. cost < 220 : 避开靠墙太近的高代价区域(膨胀层)，防 DWA 卡死
          // 2. 如果代价小，或者(代价相近且角度更直奔目标)，则作为更优选
          // 这样既能避开墙角，又不会在空旷地带绕圈
          if (cost < 160.0) { 
            if (cost < best_cost - 5.0 || (fabs(cost - best_cost) <= 5.0 && angle_diff < min_angle_diff)) { 
              best_cost = cost;
